...
- Keep image-generation conversations, runs, placements, image derivations, text-Agent conversations, per-conversation model bindings, and Skill revisions in the current PromptCard Storage schema v9. Do not embed image runs or Agent transcripts in project JSON, and do not use browser storage as an authoritative transcript source.
