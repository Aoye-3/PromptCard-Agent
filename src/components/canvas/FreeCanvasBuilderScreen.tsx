import { useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState, type ClipboardEvent as ReactClipboardEvent, type DragEvent as ReactDragEvent, type KeyboardEvent as ReactKeyboardEvent, type ReactNode } from 'react'
import {
  Background,
  BackgroundVariant,
  Controls,
...
import { ArrowLeft, Bot, ChevronRight, Image as ImageIcon, MessageSquare, MousePointer2, Palette, Pencil, Plus, Save, Trash2, Type, X } from 'lucide-react'
...
import { isFileDrag, isSupportedImageFile, uploadFreeCanvasImageFiles } from '@/components/canvas/canvas-image-assets'
import {
...
const FreeCanvasBuilderInner = ({
  ...
  const createArrow = useCallback(() => {
    addNode({
      id: `free-arrow-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      kind: 'arrow',
      title: 'Arrow',
      position: nextNodePosition(reactFlow, freeCanvas.nodes.length),
      width: 260,
      height: 120,
      text: '',
      color: '#111827',
  ...
  const handleDrop = async (event: ReactDragEvent<Element>) => {
    ...
    try {
      ...
      const uploaded = await uploadFreeCanvasImageFiles(files, position)
...
const FreeCanvasTextNodeView = ({
  ...
  return (
    ...
    >
      ...
      <div
        ref={editorRef}
        className={`nodrag nowheel min-h-[72px] whitespace-pre-wrap break-words bg-transparent font-semibold leading-snug outline-none ${editing ? 'cursor-text' : 'cursor-default'} ${fontSizeClass(node.fontSize)}`}
...
const FreeCanvasImageNodeView = ({ node, selected }: { node: IFreeCanvasImageNode; selected: boolean }) => {
  ...
  return (
    <div className={`group relative overflow-visible rounded-[10px] border bg-white p-2 shadow-[0_10px_28px_rgba(15,23,42,0.08)] ${selected ? 'border-[#c96442] ring-1 ring-[#c96442]/20' : 'border-gray-200'}`} style={{ width: node.width, height: node.height }}>
      <Handle type="target" position={Position.Left} className="!bg-gray-950 !opacity-0 group-hover:!opacity-100" />
  ...
  onUseQuickPreset: (text: string) => void
    ...
    <div className="absolute bottom-6 left-1/2 z-30 flex -translate-x-1/2 flex-col items-center gap-3">
      ...
      <div className="flex items-center gap-1 rounded-full border border-gray-200 bg-white/95 px-3 py-2 shadow-[0_18px_60px_rgba(15,23,42,0.18)] backdrop-blur" data-free-canvas-toolbar>
        ...
        <ToolbarButton title="Arrow" onClick={onCreateArrow}><MousePointer2 className="h-4 w-4" /></ToolbarButton>
      </div>
    </div>
)
