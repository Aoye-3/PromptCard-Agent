...
const CardPreviewHost = ({
  ...
  const activeProject = useMemo(() => createPreviewProject(template), [template])
  ...
  useEffect(() => {
    ...
    useCardStore.getState().restoreWorkspace({
      pages: incomingSnapshot.pages?.length ? incomingSnapshot.pages : activeProject.pages,
      currentPage: incomingSnapshot.currentPage ?? activeProject.currentPage
    })
