...
The attachment add action has exactly two sources: explicit injection of the current canvas selection and local upload. Selection injection is disabled when there is no eligible selection and never happens as a side effect of selecting, moving, connecting, restoring, or editing a canvas node. Each attachment keeps a stable `referenceId`, explicit order, source/reference role, and optional visual annotations. Reordering changes only compiled `图N`; per-image annotation actions pass the exact `referenceId` back to `FreeCanvasBuilderScreen`.
...
Never roll PromptCard Storage back below schema v6. A code rollback must preserve forward-compatible reading of conversations, runs, placements, originals, derivatives, lifecycle state, and tombstones or keep the current Storage service running until compatible code is restored.
