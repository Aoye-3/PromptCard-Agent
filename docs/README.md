...
- Keep image-generation conversations, runs, placements, image derivations, text-Agent conversations, and Skill revisions in the current PromptCard Storage schema v8. Do not embed image runs or Agent transcripts in project JSON, and do not use browser storage as an authoritative transcript source.
...
- Preserve the Canvas omnireference editing boundary: one writable target, read-only references, `@` mentions as semantics rather than permissions, append-only completion, and selection/whole-user-part rewrite with stale-proposal rejection.
