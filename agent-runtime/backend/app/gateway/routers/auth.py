...
class PromptCardBootstrapResponse(BaseModel):
    """Response model for app-managed PMAgent runtime bootstrap."""
...
@router.post("/promptcard-bootstrap", response_model=PromptCardBootstrapResponse)
async def promptcard_bootstrap(request: Request, response: Response):
    """Create or reuse the app-managed PMAgent admin and issue a session cookie.

    PMAgent embeds DeerFlow as an application module, so users should not see
    DeerFlow's standalone login form. This endpoint preserves Gateway
    session/CSRF behavior while letting the host app establish a local runtime
    session transparently.
    """
    email = os.getenv("PROMPTCARD_AGENT_ADMIN_EMAIL", "admin@promptcard.dev")
    password = os.getenv("PROMPTCARD_AGENT_ADMIN_PASSWORD", "PromptCardAgentRuntime!2026")
    provider = get_local_provider()
    ...
    return PromptCardBootstrapResponse(
        user=UserResponse(
            id=str(user.id),
            email=user.email,
